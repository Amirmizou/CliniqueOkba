...
const STR = ['title', 'title_ar', 'eventType', 'description', 'description_ar', 'location', 'location_ar', 'contact'] as const
...
export async function GET() {
  ...
  const evenements = await writeClient.fetch(
    `*[_type == "event"] | order(startDate desc) {
      _id, title, title_ar, "slug": slug.current, eventType, description, description_ar, 
      startDate, endDate, location, location_ar, registrationDeadline, contact, published, image,
      content, content_ar
    }`,
  )
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof b._rawContent === 'string') doc.content = textToBlocks(b._rawContent)
    if (typeof b._rawContent_ar === 'string') doc.content_ar = textToBlocks(b._rawContent_ar)
    ...
    revalidateSite()
    return NextResponse.json({ ok: true, evenement: result })
  } catch (e) {
    console.error('Erreur sauvegarde événement:', e)
